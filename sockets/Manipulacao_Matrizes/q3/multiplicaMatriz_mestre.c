...
int main(int argc, char const *argv[])
{
	...
	{    
		printf("Enviando o linha [%d] para [%d]\n",i+1, ntohs(escravos[0].sin_port));
		if(sendto(sock_A, matriz_A->dados[i], sizeof(int)*LEN_MATRIZ, 0, (struct sockaddr *) &escravos[0], slen_A)==-1){
	...
	{    
		printf("Enviando a matriz 2 para [%d]\n", ntohs(escravos[i].sin_port));
	...
	for (i = 0; i < LEN_MATRIZ; ++i)
	{  
		recvfrom(sock_A, matriz_aux[i], sizeof(int)*LEN_MATRIZ, 0, (struct sockaddr *) &escravos[0],&slen_A);
		printf("Pacote recebido de %s:%d\n", inet_ntoa(escravos[0].sin_addr), ntohs(escravos[0].sin_port));
		
	}
	for (i = 0; i < matriz_C->qtd_linhas; ++i)
	{
		clona_matriz(matriz_C,matriz_aux[i],i);
	}
