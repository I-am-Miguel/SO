...
int main(int argc, char const *argv[])
{
	...
	if(recvfrom(s, &matriz_A->dados[0], sizeof(int)*LEN_MATRIZ, 0, (struct sockaddr *) &mestre, &slen)==-1){
	...
	{
		if(recvfrom(s, &matriz_B->dados[i], sizeof(int)*LEN_MATRIZ, 0, (struct sockaddr *) &mestre, &slen)==-1){
			printf("ERRO AO RECEBER DADO");
			return 1;
		}
	}
